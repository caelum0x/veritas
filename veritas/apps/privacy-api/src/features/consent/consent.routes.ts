...
export function registerConsentRoutes(router: Router, deps: ConsentServiceDeps): void {
  ...
  router.post("/consents/capture", requireAuth, (req: Request, res: Response, next: NextFunction) => ctrl.capture(req, res, next));

  // POST /consents/grant — record a granted consent decision
  router.post("/consents/grant", requireAuth, (req: Request, res: Response, next: NextFunction) => ctrl.grant(req, res, next));

  // POST /consents/deny — record a denied consent decision
  router.post("/consents/deny", requireAuth, (req: Request, res: Response, next: NextFunction) => ctrl.deny(req, res, next));

  // POST /consents/withdraw — withdraw a previously granted consent
  router.post("/consents/withdraw", requireAuth, (req: Request, res: Response, next: NextFunction) => ctrl.withdraw(req, res, next));

  // GET /consents/check — check if user has an active grant for a purpose (before /consents)
  router.get("/consents/check", requireAuth, (req: Request, res: Response, next: NextFunction) => ctrl.check(req, res, next));

  // GET /consents — list consents for a user
  router.get("/consents", requireAuth, (req: Request, res: Response, next: NextFunction) => ctrl.list(req, res, next));
}
